...
The high CVSS score reflects that SSH brute force requires no prior access, no special conditions, and targets a service frequently exposed on internet-facing infrastructure.
...
No systems were compromised. The dvwa-vm was the intended target. All other systems remained unaffected throughout the incident.
...
No software vulnerability was exploited. The attack targeted the SSH authentication mechanism through credential guessing. The target VM mitigated the attack by having `PasswordAuthentication` disabled - only SSH key-based authentication was accepted.

### Why the Attack Was Unsuccessful

The dvwa-vm was configured with `PasswordAuthentication no` in `/etc/ssh/sshd_config`. The SSH daemon rejected all password-based authentication attempts before they could be evaluated. Hydra received authentication failure responses for every attempt.
...
All auth.log events were shipped by Filebeat, processed by Logstash, and indexed in Elasticsearch. The following KQL query in Kibana Discover returns all events from this incident:
...
| Rule Name | Threshold | Window | Fired |
| --- | --- | --- | --- |
| SSH Brute Force Detection | More than 5 failures | 60 seconds | Yes |
| SSH Slow Brute Force Detection | More than 10 failures | 10 minutes | Yes |

![Kibana detection rules status](screenshots/01-kibana-rule-status.png)

### Automated Response Evidence

The Python alert engine detected the attack and blocked the source IP automatically:

![Python alert engine output](screenshots/01-python-engine-alert.png)

### Azure NSG Block Rule

The deny rule was automatically added to the NSG within 5 seconds of threshold breach:
...
| Incident closed | June 4, 2026 22:00 UTC | Resolved |
...
| KQL Query | `message: "Invalid user" AND host.name: "dvwa-vm"` | Kibana detection |
...
| Secondary Cause | SSH port 22 accessible from all subnet VMs rather than jump-box only |
| Contributing Factor | No network-level rate limiting prior to Python alert engine deployment |
| Why Attack Failed | PasswordAuthentication was disabled - no password mechanism to exploit |
...
System integrity was verified by confirming all services were operational and no unexpected processes, files, or accounts were present on dvwa-vm following the incident.
...
- Enable MFA for all administrative SSH access
...
- Slow-and-low attacks evaded the original 60-second threshold rule - mitigated by adding a 10-minute window rule
...
- Apache logs added to Filebeat configuration to detect web application scanning
